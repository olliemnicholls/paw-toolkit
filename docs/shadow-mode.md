...
`@compile_on_hit` used to route calls to a compiled adapter the moment the compile
finished. The project's own measurement showed why that is not safe: a real ticket-triage
adapter replaced a live Claude teacher at ~11x lower latency and agreed with a fresh
teacher call on only 60% of tickets. Shadow mode is the gate in front of that swap. It is
on by default.
...
- **`shadow`**: the compile finished. Your function still serves every call. Off the
  ...
  window is evaluated exactly once, so an adapter that agrees 60% of the time does not get
  a fresh draw on every call.
...
The mock quickstart, `paw-kit demo`, the three examples and the measurement scripts all
pass `shadow_window=0`. A five-call demo has nowhere near enough calls to fill a window,
and the recorded measurements predate shadow mode and must stay reproducible. Measuring
shadow mode itself, including what `audit_rate=0.05` costs in teacher calls, is a
follow-up.

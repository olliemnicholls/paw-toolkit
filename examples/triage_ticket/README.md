...
This example demonstrates how to use the `@compile_on_hit` decorator from `paw-kit` to automatically compile a high-volume LLM prompt into a local, zero-marginal-cost neural function.
...
3. **Calls 6 to 10 (Hot-Swap to Local Neural Function)**: Once compiled, subsequent calls are automatically redirected to the local adapter, dropping latency from ~200ms to <1ms and eliminating API token costs entirely.
